...
function addHoverVideoBehavior(selector) {
    document.querySelectorAll(selector).forEach(container => {
        const video = container.querySelector('.ranger-video');
        ...
        container.addEventListener('mouseenter', () => {
            video.play(); // Play the video on mouse enter
            container.classList.add('active');
        });

        container.addEventListener('mouseleave', () => {
            video.pause(); // Pause playback on mouse leave
            video.currentTime = 0; // Reset playback time
